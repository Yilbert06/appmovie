...
export const getDirectorsThunk = () => (dispatch) => {
  dispatch(
    genericRequestThunk(async () => {
      const res = await axios.get("https://movie-app-pxjy.onrender.com/api/v1/directors");
  ...
  dispatch(
    genericRequestThunk(async () => {
      const res = await axios.post(
        "https://movie-app-pxjy.onrender.com/api/v1/directors",
        director
      );
...
export const deleteDirectorThunk = (id) => (dispatch) => {
  dispatch(
    genericRequestThunk(async () => {
      await axios.delete(`https://movie-app-pxjy.onrender.com/api/v1/directors/${id}`);
  ...
  dispatch(
    genericRequestThunk(async () => {
      const { data: director } = await axios.put(
        `https://movie-app-pxjy.onrender.com/api/v1/directors/${id}`,
        directorParams
      );
